...
\newcommand{\source}{
	\draw (40*\xscale,\yscale) node[anchor=south west,ctcoloraccessory]{\Var{source}};
}
\newcommand{\target}{
	\draw (0,\yscale) node[anchor=south east,ctcoloraccessory]{\Var{target}};
}
\newcommand{\border}{
...
\begin{document}
	\begin{tikzpicture}	
		...
		\target
		\border
		\draw (-\xscale,\yscale/2) node[left]{\textbf{(1)}};
	\end{tikzpicture}
\end{document}