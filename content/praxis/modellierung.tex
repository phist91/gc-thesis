...
Nach einer kurzen Beschreibung der Anforderungen an diese Anwendung erfolgt ein Blick auf die Modellierung und Implementation der zentralen Programmkomponenten, in der die wichtigsten Designentscheidungen erläutert werden.
...
Diese können über die grafische Oberfläche verändert werden (siehe Abschnitt~\ref{sec:execution}) und werde beim Beenden in einer Datei \code{gcsim.config} gesichert bzw. aus dieser geladen werden.
...
Diese besitzt neben einer Referenz auf die Menge \code{objects} und der aktuellen Position des Allokators zusätzlich die innere Klasse \code{CanvasPanel} als Spezialisierung der Swing-Komponente \code{JPanel} (siehe Listing~\ref{java:canvas}).
...
Ein manueller Aufruf ist durch den \code{CollectionController} über die Methode \code{refreshFrame} (Zeile~2 bis~6) möglich.
