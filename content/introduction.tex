...
\begin{proof}
	Dies ist ein Korollar aus der Unentscheidbarkeit des Halteproblems. \todo{evtl. noch ausführen?}
\end{proof}
...
Besitzt ein Objekt \code{a} in mindestens einem seiner Felder eine Referenz auf ein weiteres Objekt \code{b}, so wird dies durch eine gerichtete Kante von \code{a} nach \code{b} dargestellt.
...
An dieser Stelle mag man sich fragen, warum wir nicht voraussetzen, dass die Ausführung eines Garbage-Collection-Algorithmus nicht die Freigabe \textit{sämtlicher} nicht erreichbaren Objekte anfordert.
