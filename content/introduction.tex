...
Wir fokussieren dabei zunächst drei Klassen von Algorithmen, die als Grundlage aller fortgeschrittenen Konzepte betrachtet werden können: markierende Algorithmen, referenzzählende Algorithmen und objektkopierende Algorithmen zur Kompaktierung des Heaps (vgl. \cite[S. 25]{blackburn2004}).
Dazu wird aus jeder Klasse zunächst ein elementarer Algorithmus betrachtet, an dem das Prinzip der Algorithmen nachvollzogen werden kann.
...
\todo[inline]{ggfs. noch überarbeiten}
...
Den Zugriff auf das \Var{i}-te Feld eines Objekts \Var{a} notieren wir -- analog zur Syntax der Programmierspache C -- mit \Var{a[i]}.
Ebenso bezeichnen wir mit \Var{\&a} die Adresse eines Objekts und mit \Var{*p} die Dereferenzierung eines Zeigers \Var{p}.
...
\begin{defn}[Erreichbarkeit]
	...
	\end{enumerate}
	Gilt $\Var{*a[i]} = \Var{b}$, so schreiben wir auch $\Var{a} \rightarrow \Var{b}$.
	Existiert für zwei Objekte $\Var{a}, \Var{b}$ eine endliche Folge von Objekten $\Var{a}_1, \dots, \Var{a}_n$ mit $\Var{a} \rightarrow \Var{a}_1 \rightarrow \dots \rightarrow \Var{a}_n \rightarrow \Var{b}$, so notieren wir dies zudem mit $\Var{a} \transreach \Var{b}$.
\end{defn}
\end{mybox}
...
Besitzt ein Objekt \Var{a} in mindestens einem seiner Felder eine Referenz auf ein weiteres Objekt \Var{b}, so wird dies durch eine gerichtete Kante zwischen den ensprechenden Knoten dargestellt.
