...
Algorithmen, die im Rahmen der Garbage Collection die Fragmentierung des Heaps beseitigen oder verhindern, lassen sich grob in zwei Kategorien einteilen:
...
Weiter soll eine Prozedur \Method{moveObject}(\Var{old}, \Var{new}) existieren, die das Objekt an der Speicheradresse \Var{old} an die Speicheradresse \Var{new} verschiebt, wobei eine Überschreibung von Daten an der Zielposition ausdrücklich zugelassen ist.
...
Dabei wird für jedes verschobene Objekt die alte und neue Speicheradresse in einer Tabelle \Var{log} hinterlegt (Zeile~6 in Algorithmus~\ref{algo:lisp2gc}).
...
\begin{figure}[H]
	\centering
	\includestandalone[scale=0.85]{img/tikz/ch4-lisp2}
...
\begin{satz}[Komplexität des LISP-2-Algorithmus]
	...
	\begin{enumerate}[(1)]
		...
		\item Die Laufzeit von \Method{updateRefs} liegt in $\oh\enb{\sum\limits_{\Var{a} \in \Roots}^{} \abs{\Fields(a)} + \abs{\HeapSize}}$.
	\end{enumerate}
\end{satz}
\end{mybox}
...
Wurden alle erreichbaren Objekte erfasst, werden auch die Zeiger \Var{pos} und \Var{next} im LISP-2-Algorithmus korrekt weitergesetzt, ohne dass es zur Überschreibung erreichbarer Objekte kommt.
...
%	\State \quad $\Var{limit} \gets \Var{target} + \Var{spaceSize}$
	...
	\State \quad \quad $\Method{moveObject}(\Var{oldAdr}, \Var{newAdr}(\Var{oldAdr}))$ \Comment{Kopiere Objekt an neue Position}
...
Ein dritter Zeiger \Var{pos} zeigt den Füllstand von \Var{target} und damit die Position an, an der ein neues Objekt angelegt bzw. kopiert wird.
Der Allokator ist entsprechend so zu modifizieren, dass neue Objekte an der Stelle \Var{pos} angelegt werden und \Var{pos} anschließend angepasst wird, sowie die Garbage Collection bereits dann ausgelöst wird, wenn der freie Speicher in \Var{target} zur Neige geht.
...
Falls nicht, wird die neue Adresse des Objekts auf die Position von \Var{pos} gesetzt, das Objekt an diese Stelle kopiert und \Var{pos} hinter das Objekt gesetzt (Zeile~18 bis~20).
...
Nachdem die Rollen der Halbräume \Var{source} und \Var{target} vertauscht wurden, wird zunächst das einzige von Basisobjekten referenzierte Objekt \Var{b} kopiert \textbf{(2)}.
...
\begin{proof}
	...
	Dann gilt $\Var{b[i]} = \Var{\&a}$ für Feld \Var{b[i]} eines Objekts \Var{b}.
...
Alle Objekte besitzen anstelle einer Referenz auf \Var{a} eine solche auf \handle{a}.
...
Insofern sollte der Zugriff auf \Var{handle}(\Var{a}) nur dem Kollektor ermöglicht werden.
