...
\label{sec:naive-rc}
Zur Realisierung des Ansatzes von \textsc{Collins} im eingangs betrachteten Speichermodell wird erneut der Header eines Objekts \Var{a} genutzt, um einen ganzzahligen, nicht negativen Wert \Var{rc}(\Var{a}) zu hinterlegen, der als \textbf{Referenzzähler} von \Var{a} bezeichnet wird.
...
Um dies zu überprüfen, wird \Method{scan} rekursiv für alle Objekte von $\sk{\Var{obj}}$ aufgerufen (Zeile~11 bis 13).
...
An dieser Stelle ist bereits erkennbar, dass alle Referenzzähler von $\sk{\Var{b}}$ $0$ sind, weswegen die starke Zusammenhangskomponente keine externen Referenzen besitzt und nicht mehr erreichbar ist.
...
\begin{algorithmic}[1]
	...
	\State \quad \FOREACH $\Var{obj} \in \Var{ZCT}$		\Comment{nicht erreichbare Objekte löschen}
	\State \quad \quad \FOREACH $\Var{ref} \in \Pointers(\Var{obj})$
	\State \quad \quad \quad \Method{decRefCount}(\Var{*ref})
	\State \quad \quad \Method{free}(\Var{obj})
...
Da die Objekte in der \Var{ZCT} gegebenenfalls noch von lokalen Variablen referenziert werden und in diesem Fall nicht entfernt werden dürfen, müssen zunächst ihre Referenzzähler angepasst werden, indem über alle Referenzen von Basisobjekten iteriert wird (Zeile~20).
