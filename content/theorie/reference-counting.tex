...
\label{sec:naive-rc}
Zur Realisierung des Ansatzes von \textsc{Collins} im betrachteten Speichermodell wird erneut der Header eines Objekts \Var{a} genutzt, um einen ganzzahligen, nicht negativen Wert \Var{rc}(\Var{a}) zu hinterlegen, der als \textbf{Referenzzähler} von \Var{a} bezeichnet wird.
...
Speichermangel kann so de facto nur dadurch entstehen, dass der gesamte Speicher von erreichbaren Objekten belegt wird -- in diesem Fall bietet allerdings kein Garbage-Collection-Algorithmus Abhilfe -- oder der Heap zunehmend fragmentiert wird (siehe Kapitel~\ref{cha:compacting}).
...
Nachfolgend wird ein Algorithmus von \textsc{Martínez}, \textsc{Wachenchauzer} und \textsc{Lins} vorgestellt, der die naive Referenzählung um ein Verfahren ergänzt, mit welchem zyklische Strukturen zuverlässig erkannt werden können \cite{martinez1990}.
...
\begin{algorithmic}[1]
	...
	\State \quad \quad $\Var{rc}(\Var{*ref}) \gets \Var{rc}(\Var{*ref})+1$ \Comment{Interne Referenz wiederherstellen}
...
An dieser Stelle ist bereits erkennbar, dass alle Referenzzähler von $\sk{\Var{b}}$ $0$ sind, weswegen der Zyklus keine externen Referenzen besitzt und nicht mehr erreichbar ist.
...
Da die Objekte in der \Var{ZCT} gegebenenfalls noch von lokalen Variablen referenziert werden und nicht entfernt werden dürfen, müssen zunächst ihre Referenzzähler angepasst werden, indem über alle Referenzen von Basisobjekten iteriert wird (Zeile~20).
Objekte, die im Anschluss in \Var{ZCT} verbleiben, sind tatsächlich verwaist und können bedenkenlos entfernt werden, ohne die Korrektheit des Algorithmus zu gefährden (Zeile~22 bis~26).
