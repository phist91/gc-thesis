...
\label{sec:naive-rc}
Zur Realisierung des Ansatzes von \textsc{Collins} im eingangs betrachteten Speichermodell wird erneut der Header eines Heappbjekts \Var{a} genutzt, um einen ganzzahligen, nicht negativen Wert \Var{rc}(\Var{a}) zu hinterlegen, der als \textbf{Referenzzähler} von \Var{a} bezeichnet wird.
...
Immer, wenn eine Referenz auf \Var{a} in ein Feld eines anderen Objekts geschrieben wird, muss \Var{rc}(\Var{a}) inkrementiert werden.
...
\begin{algorithmic}[1]
	...
	\State \quad \quad $\Var{rc}(\Var{*ref}) \gets \Var{rc}(\Var{*ref}) + 1$ \CommentCont{Selbstreferenz geschrieben wird}
	\State \quad \IF (\Var{obj[i]} $\neq$ \Null $\wedge$ \Var{*obj[i]} $\neq$ \Var{obj})	\Comment{Reduziere \Var{rc}, falls nicht \Null oder}
	\State \quad \quad \Method{decRefCount}(\Var{*obj[i]})	\CommentCont{Selbstreferenz überschrieben wird}
	...
	\State \quad \quad \quad \quad \Method{decRefCount}(\Var{*ref}) \CommentCont{referenzierter Objekte}
...
Die in Algorithmus~\ref{algo:naive-rc} aufgeführte Schreibbarriere kommt genau dann zum Einsatz, wenn durch den Mutator mittels $\Var{obj[i]} \gets \Var{ref}$ in ein Feld eines Objekts eine Referenz auf ein Heapobjekt geschrieben wird.
...
Das Verringern des Referenzzählers wird durch die Prozedur \Method{decRefCount} übernommen.
...
Im Gegensatz zum Mark-Sweep-Algorithmus finden kaum unvorhersehbare Objektzugriffe statt.
Wird der Referenzzähler eines Objekts erhöht, so wird gerade eine neue Referenz hierauf angelegt.
...
\end{algorithmic}
\caption[Lineare Suche]{Lineare Suche in einer verketteten Liste. Obwohl der Algorithmus keine Objekte manipuliert, verursacht jede Änderung der Referenzvariable \Var{cur} die Manipulation von zwei Referenzzählern.}
...
Algorithmus~\ref{algo:cyclic-rc} ergänzt die Prozedur \Method{decRefCount} der naiven Referenzzählung um einen \ELSE-Fall, der ausgelöst wird, wenn der Referenzzähler \Var{rc}(\Var{obj}) nach Dekrementierung nicht $0$ beträgt (vgl. Zeile~7 bis 10).
In diesem Fall muss überprüft werden, ob \Var{obj} das letzte Objekt einer starken Zusammenhangskomponente war, durch welches diese erreichbar war.
...
\begin{algorithmic}[1]
	...
	\State \quad \quad \FOREACH $\Var{ref} \in \Pointers(\Var{obj})$	\Comment{Entferne interne Referenz,}
	\State \quad \quad \quad $\Var{rc}(\Var{*ref}) \gets \Var{rc}(\Var{*ref})-1$ \CommentCont{die von \Var{obj} ausgeht}
	\State \quad \quad \quad \Method{markGray}(\Var{*ref})	\Comment{Reduziere rekursiv weitere \Var{rc}}
	\Statex
	...
	\State \quad \quad \quad \quad \Method{scan}(\Var{*ref})	\Comment{Prüfe rekursiv referenzierte Objekte}
...
Besitzt \Var{rc}(\Var{obj}) den Wert $0$, so wird \Var{obj} zunächst schwarz markiert (Zeile~9f).
...
Um dies zu überprüfen, wird \Method{scan} rekursiv für alle Objekte von $\sk{\Var{obj}}$ aufgerufen (Zeile~11f).
...
weswegen \Method{unmark} rekursiv für diese Objekte aufgerufen wird (Zeile~6).
...
\begin{algorithmic}[1]
	...
	\State \quad \quad $\Var{rc}(\Var{*ref}) \gets \Var{rc}(\Var{*ref})+1$ \Comment{Interne Referenz wieder berücksichtigen}
	\State \quad \quad \IF \NOT \Method{isWhite}(\Var{*ref})
	\State \quad \quad \quad \Method{unmark}(\Var{*ref})
	\Statex
	\State \MethodHead{collect}(\Var{obj}):
	\State \quad \IF \Method{isBlack}(\Var{obj})	\Comment{Objekt ist nicht erreichbar}
	\State \quad \quad \FOREACH $\Var{ref} \in \Pointers(\Var{obj})$
	\State \quad \quad \quad \Method{collect}(\Var{*ref})
	\State \quad \quad \Method{free}(\Var{obj})
\end{algorithmic}
...
Um die Arbeit des Algorithmus zu veranschaulichen, wird nun exemplarisch der Objektgraph aus Abbildung~\ref{fig:rc-scc} betrachtet und angenommen, dass die Referenz $\Var{a} \rightarrow \Var{b}$ entfernt wird, wodurch die Objekte \Var{b}, \Var{c}, \Var{d} und \Var{e} unerreichbar werden.
Zu erwarten ist, dass der Aufruf von \Method{markGray}(\Var{b}), \Method{scan}(\Var{b}) und \Method{collect}(\Var{b}) alle vier Objekte freigibt.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption[Ausführung von \Method{markGray}(\Var{b})]{Beispielhafte Ausführung von \Method{markGray}(\Var{b}) nach Entfernen der Referenz $\Var{a} \rightarrow \Var{b}$. Dick umrandet sind diejenigen Objekte, für welche aktuell der rekursive Aufruf von \Method{markGray} abgearbeitet wird.}
...
Dazu wird das Beispiel um eine externe Referenz auf das Objekt \Var{d} ergänzt (siehe Abbildung~\ref{fig:rc-unmark-example}).
Nach Terminierung von \Method{markGray}(\Var{b}) weist \Var{rc}(\Var{d}) dann den Wert $1$ auf.
Durch den Aufruf \Method{scan}(\Var{b}) werden zunächst \Var{b}, \Var{e} und \Var{c} schwarz markiert.
...
\begin{proof}
	Wir zeigen zunächst, dass die Aufrufe der Prozeduren \Method{markGray}, \Method{scan} und \Method{collect} (Zeile~8 bis~10 in Algorithmus~\ref{algo:cyclic-rc}) terminieren:
	...
	Ist der Referenzzähler eines Objekts $0$, für welches \Method{scan} aufgerufen wird, so wird dieses schwarz gefärbt.
	...
	Wird nun ein Objekt \Var{a} durch \Method{collect} freigegeben, so wurde \Var{a} zuvor schwarz markiert (Zeile~8 und~11 in Algorithmus~\ref{algo:cyclic-clean}).
	...
	In diesem Fall wird \Var{b} jedoch durch \Method{unmark} weiß markiert und rekursiv auch \Var{a}, da $\Var{b} \transreach \Var{a}$ gilt.
...
\begin{algorithmic}[1]
	...
	\State \quad \quad \quad \Method{decRefCount}(\Var{*obj[i]})
	\State \quad \Var{obj[i]} $\gets$ \Var{ref}
	\Statex
	\State \MethodHead{incRefCount}(\Var{obj}):
	\State \quad \IF $\Var{obj} \in \Var{ZCT}$
	\State \quad \quad \Method{remove}(\Var{ZCT}, \Var{obj})	\Comment{Anzahl Referenzen ist nun $1$}
	...
	\State \quad \ELSE \Method{add}(\Var{MRT}, \Var{obj})		\Comment{Anzahl Referenzen ist nun $2$}
	\Statex
	\State \MethodHead{decRefCount}(\Var{obj}):
	...
	\State \quad \quad \ELSE \Method{remove}(\Var{MRT}, \Var{obj})	\Comment{Anzahl Referenzen ist nun $1$}
	\State \quad \ELSE \Method{add}(\Var{ZCT}, \Var{obj})			\Comment{Anzahl Referenzen ist nun $0$}
	\Statex
	\State \Atomic \MethodHead{collect}():
	\State \quad \FOREACH $\Var{ref} \in \Pointers(\Roots)$		\Comment{nicht gezählte Referenzen hinzufügen}
	\State \quad \quad \Method{incRefCount}(\Var{*ref})
	\State \quad \WHILE $\Var{ZCT} \neq \emptyset$		\Comment{nicht erreichbare Objekte löschen}
	...
	\State \quad \quad \Method{decRefCount}(\Var{*ref})
\end{algorithmic}
...
Wird nun eine Referenz \Var{ref} in ein Feld \Var{obj[i]} geschrieben, kann der Schreibzugriff unsynchronisiert ausgeführt werden, sofern \Var{obj} ein Basisobjekt ist, da keine Referenzzähler angepasst werden (Zeile~2 in Algorithmus~\ref{algo:deferred-rc}).
Andernfalls sind mehrere Fälle zu unterscheiden:
Wird ein Referenzzähler eines Objekts \Var{obj} erhöht, das sich in \Var{ZCT} befindet (etwa, da es bislang nur von lokalen Variablen referenziert wird), so muss es aus \Var{ZCT} entfernt werden, da der Referenzzähler nun $1$ beträgt (Zeile~8f).
Ist es in der \Var{MRT}, so ist der zugehörige Referenzzähler $\Var{MRT}(\Var{obj})$ zu erhöhen (Zeile~10f).
Ist beides nicht der Fall, so muss \Var{obj} zur \Var{MRT} hinzugefügt werden; der Referenzzähler wird entsprechend mit $2$ initialisiert (Zeile~12).
...
Da die Objekte in der \Var{ZCT} gegebenenfalls noch von lokalen Variablen referenziert werden und in diesem Fall nicht entfernt werden dürfen, müssen zunächst ihre Referenzzähler angepasst werden, indem über alle Referenzen von Basisobjekten iteriert wird (Zeile~20f).
Objekte, die im Anschluss in der \Var{ZCT} verbleiben, sind tatsächlich verwaist und können bedenkenlos entfernt werden, ohne die Korrektheit des Algorithmus zu gefährden (Zeile~22 bis~26).
...
Je nach Implementation der \Var{ZCT} können außerdem Löschkaskaden, die während einer Kollektionsphase entstehen können, auf den nächsten Kollektionszyklus verschoben werden, um Unterbrechungen minimal zu halten. 
...
Mittels Methoden für verteilte Systeme, auf die hier nicht näher eingegangen wird, lässt sich dieser Ansatz durch feingranularere Synchronisationen optimieren, etwa indem Logtabellen threadweise angelegt werden.
...
\begin{algorithmic}[1]
	...
	\State \quad \quad \quad \Method{incRefCount}(\Var{**field})	\Comment{vgl. Algorithmus~\ref{algo:deferred-rc}}
	\State \quad \quad \quad \Method{decRefCount}(\Var{*log}(\Var{field}))	\Comment{vgl. Algorithmus~\ref{algo:deferred-rc}}
...
Mit dem Mark-Sweep-Algorithmus und der Referenzzählung haben wir nun zwei zunächst diametral verschiedene Ansätze zur automatischen Speicherverwaltung kennengelernt.
