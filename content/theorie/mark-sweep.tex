...
\begin{algorithmic}[1]
	...
	\State \quad \quad \quad \Method{free}(\Var{blk})	\Comment{Gib komplett verwaiste Region sofort frei}
	...
	\State \quad \Method{createBlock}(\Var{size})	\Comment{Initialisiere neue Region, wenn nichts freigegeben}
\end{algorithmic}
...
Hingegen werden Regionen, die mindestens ein erreichbares Objekt enthalten und somit markiert sind, nach Abschluss der Markierungsphase einer Menge \Var{toSweep} zugefügt (Zeile~18).
...
Dabei werden nach und nach Regionen aus \Var{toSweep} entfernt und bereinigt, die Objekte der angeforderten Speichergröße verwalten.
...
Während die Einschränkung der Bereinigung auf einzelne Regionen zwar einen Performancevorteil mit sich bringt, besitzt diese Variante des Mark-and-Sweep-Algorithmus auch einige Nachteile:
...
\section{Weitere Varianten und Vergleich}
...
Anstatt etwa im naiven Algorithmus~\ref{algo:naive-mark} die Abarbeitung der \Var{toDo}-Menge zu beginnen, sobald das erste Basisobjekt erfasst wurde, können statt dessen auch zunächst alle Basisobjekte zu \Var{toDo} hinzugefügt und die Methode \Method{mark} im Anschluss aufgerufen werden.
...
Da wir bereits festgestellt haben, dass keine Objekte mehrfach zu \Var{toDo} hinzugefügt werden, ist ihre Mächtigkeit durch die Anzahl $|\ObjectSet|$ aller Objekte beschränkt.
Davon ausgehend, dass in \Var{toDo} ausschließlich Referenzen konstanter Größe gespeichert werden, ergibt sich insgesamt ein Platzbedarf von $\oh(|\ObjectSet|)$.
...
Im schlimmsten Fall müssen alle existierenden Objekte und jede Referenz verfolgt werden, um auszuschließen, dass ein erreichbares Objekt nicht unmarkiert bleibt.
...
\begin{satz}[Speicherbedarf und Komplexität des Mark-Sweep-Algorithmus]
	...
	\begin{enumerate}[(1)]
		\item Der Speicherbedarf des gesamten Algorithmus ist $\oh(|\ObjectSet|)$, wobei $\ObjectSet$ die Menge aller Objekte ist.
		\item Die Laufzeit der Markierungsphase ist $\oh\enb{|\ObjectSet|+\sum\limits_{o \in \ObjectSet}^{} \abs{\Pointers(o)}}$.
